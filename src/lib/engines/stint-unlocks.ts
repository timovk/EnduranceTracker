...
export async function reconstructStintUnlocks(
  ...
  return {
    ...
    // In the order the stint itself listed them: ladder by ladder, rung by rung.
    milestones: milestones
      .sort((a, b) => ladderIndex(a.metric) - ladderIndex(b.metric) || a.threshold - b.threshold)
      .map((m) => ({
        metric: m.metric,
...
/** Where a ladder sits among the ladders; rows of no ladder go last. */
function ladderIndex(metric: string): number {
  const index = MILESTONES.findIndex((def) => def.metric === metric);
  return index < 0 ? MILESTONES.length : index;
}
