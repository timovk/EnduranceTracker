 ...
 * first dating does (not after the app recorded it) and differs from what the
 * row already holds; every other row, dated or not, is left exactly as it is.
 * Returns how many rows were rewritten, so a second rebuild reports 0.
 */
export async function rebuildLandmarkDates(tx: Tx, userId: string, timeline: CareerTimeline): Promise<number> {
  const [milestoneRows, stepRows] = await Promise.all([
    tx.milestoneProgress.findMany({
      ...
      select: {
        id: true, metric: true, threshold: true, reachedAt: true,
        achievedAt: true, achievedPrecision: true, sessionId: true, raceId: true, eventId: true, subjectName: true,
      },
    }),
    tx.masteryProgress.findMany({
      where: { userId, unlockedAt: { not: null }, node: { tree: { kind: 'RACE_EVENT' } } },
      select: {
        id: true, unlockedAt: true, achievedAt: true, achievedPrecision: true, achievedSessionId: true,
        node: { select: { metric: true, threshold: true, tree: { select: { iconicKey: true } } } },
      },
    }),
  ]);
  const slack = TIMELINE_SHAPE.recognitionSlackMinutes;
  const sameInstant = (stored: Date | null, replayed: Date) => stored !== null && stored.getTime() === replayed.getTime();
  let rewritten = 0;
  ...
  for (const row of milestoneRows) {
    ...
    if (
      sameInstant(row.achievedAt, data.achievedAt) && row.achievedPrecision === data.achievedPrecision &&
      row.sessionId === data.sessionId && row.raceId === data.raceId && row.eventId === data.eventId &&
      row.subjectName === data.subjectName
    ) continue;
    const written = await tx.milestoneProgress.updateMany({ where: { id: row.id, userId }, data });
  ...
  for (const row of stepRows) {
    ...
    if (
      sameInstant(row.achievedAt, instant.at) && row.achievedPrecision === instant.precision &&
      row.achievedSessionId === instant.sessionId
    ) continue;
    const written = await tx.masteryProgress.updateMany({
