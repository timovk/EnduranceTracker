...
describe('deleting a stint from the first year', () => {
  it('re-stamps the ledger in batches and keeps I2', async () => {
    ...
    expect(await ledgerProblems(USER)).toEqual([]);
    const later = async () => prisma.xPTransaction.findMany({
      where: { userId: USER, source: 'CHALLENGE' },
      orderBy: [{ createdAt: 'asc' }, { id: 'asc' }],
      select: { careerXpAfter: true },
    });
    ...
    // Every one of the thousand rows after it — more than two batches — was
    // re-stamped by exactly what came off.
    const after = await later();
    expect(after.map((row) => Number(row.careerXpAfter)))
      .toEqual(before.map((row) => Number(row.careerXpAfter) - removal.careerXpRemoved));
    expect(await ledgerProblems(USER)).toEqual([]);
  });
});
