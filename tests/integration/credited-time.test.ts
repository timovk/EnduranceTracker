...
describe('credited time', () => {
  it('credits a stint played at 0.1× as its timeline at 0.75×, everywhere', async () => {
    ...
    // The race page: its "Real viewing" figure, and the stint's row under it,
    // which shows the same credited time, so the rows add up to the figure.
    const detail = await getRaceDetail(USER, raceId);
    expect(detail?.creditedViewingSec).toBe(80 * 60);
    expect(detail?.sessions.map((session) => session.creditedSeconds)).toEqual([80 * 60]);
  ...
  it('is simply real time at 0.75× and faster', async () => {
    ...
    for (const session of detail?.sessions ?? []) expect(session.creditedSeconds).toBe(session.realSeconds);
  });
...
describe('events', () => {
  ...
  it('records a step worth nothing as reached, and writes no ledger row for it', async () => {
    const raceId = await addRace(USER, { iconicKey: 'zero-step', raceDate: new Date(Date.UTC(2026, 5, 13)) });
    await logStint(USER, raceId, { from: 0, to: H, now: NOW });
    // A step that is shown and dated but paid by something else, as Event
    // Legacy's first step will be.
    const node = await prisma.masteryNode.findFirstOrThrow({
      where: { key: 'edition_1', tree: { userId: USER, key: 'event:zero-step' } },
      select: { id: true },
    });
    await prisma.masteryNode.update({ where: { id: node.id }, data: { xpReward: 0 } });
    await prisma.race.update({ where: { id: raceId }, data: { storyCompletedAt: NOW } });

    const unlocks = await prisma.$transaction((tx) => syncMastery(tx as Tx, USER, NOW));

    expect(unlocks.find((unlock) => unlock.nodeKey === 'edition_1')).toMatchObject({ xpAwarded: 0 });
    const progress = await prisma.masteryProgress.findUniqueOrThrow({
      where: { userId_nodeId: { userId: USER, nodeId: node.id } },
    });
    expect(progress.unlockedAt).toEqual(NOW);
    expect(await prisma.xPTransaction.count({ where: { userId: USER, dedupeKey: 'mastery:event:zero-step:edition_1' } }))
      .toBe(0);
    expect(await prisma.xPTransaction.count({ where: { userId: USER, amount: 0 } })).toBe(0);
  });
